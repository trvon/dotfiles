...
const CRITIC_MODEL = process.env.PI_RESEARCH_CRITIC_MODEL || "mistralai/ministral-3-14b-reasoning";
const CRITIC_MAX_TOKENS = parsePositiveInt(process.env.PI_RESEARCH_CRITIC_MAX_TOKENS, 900);

const TRACE_FILE = process.env.PI_RESEARCH_TRACE_FILE || "";
...
function parseDcsStatusModel(stdout: string): string {
  const match = stdout.match(/model=([^\s]+)/i);
  return match?.[1]?.trim() || "";
}

function parseDcsRunModels(stdout: string): { executor: string; critic: string } {
...
async function runDcs(
  ...
  trace("dcs_exec", { args });
  return pi.exec("uv", ["run", "--project", DCS_ROOT, "dcs", ...args], {
...
async function gatherWithDcs(
  ...
  const result = await runDcs(pi, ["run", task, "--context-profile", DCS_CONTEXT_PROFILE, "--ground-truth-mode", "--dspy-faithfulness"], signal);
  const raw = `${result.stdout || ""}\n${result.stderr || ""}`.trim();
  const output = extractDcsOutput(result.stdout || result.stderr || "");
  return { output: output.trim(), raw, ok: result.code === 0 };
}
...
export default function researchOrchestratorExtension(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    setStatus(ctx);
    trace("session_start", { dcsRoot: DCS_ROOT, runs: state.runs });
    notify(ctx, "Research orchestrator ready. Commands: /research-status /research-gather /research-critic /research-review");
  });

  pi.registerCommand("research-status", {
    description: "Check DCS + YAMS research stack health",
    handler: async (_args, ctx) => {
      const [yams, dcs] = await Promise.all([
        pi.exec("yams", ["status"], { timeout: 30_000 }),
        runDcs(pi, ["status"]),
      ]);

      const yamsOk = yams.code === 0;
      const dcsOk = dcs.code === 0;
      const dcsModel = parseDcsStatusModel(dcs.stdout || "");
      trace("status", { yamsOk, dcsOk, dcsModel });

      const msg = [
        `research status: yams=${yamsOk ? "ok" : "fail"} dcs=${dcsOk ? "ok" : "fail"}${dcsModel ? ` model=${dcsModel}` : ""}`,
        !yamsOk ? truncate(yams.stderr || yams.stdout || "", 220) : "",
        !dcsOk ? truncate(dcs.stderr || dcs.stdout || "", 220) : "",
      ]
        .filter(Boolean)
        .join("\n");

      notify(ctx, msg, yamsOk && dcsOk ? "info" : "warning");
    },
  });

  pi.registerCommand("research-gather", {
    description: "Run DCS gather phase for literature review topic",
    handler: async (args, ctx) => {
      ...
      const runModels = parseDcsRunModels(gathered.raw);
      state.topic = topic;
      state.gatherOutput = gathered.output;
      state.gatherModel = runModels.executor || "dcs-executor-unknown";
      state.gatherCriticModel = runModels.critic || "dcs-critic-unknown";
      state.runs += 1;
      persist();
      setStatus(ctx);
      ...
      notify(
        ctx,
        `Research gather complete (${gathered.output.length.toLocaleString()} chars). executor=${state.gatherModel} critic=${state.gatherCriticModel}. Use /research-critic next.`
      );
    },
  });
  ...
  pi.registerCommand("research-pack", {
    ...
    handler: async (_args, ctx) => {
      ...
      const packet = [
        ...
        `DCS critic model: ${state.gatherCriticModel || "unknown"}`,
  ...
  pi.registerCommand("research-review", {
    ...
    handler: async (args, ctx) => {
      ...
      trace("review_queued", {
        topic,
        dcsExecutor: state.gatherModel,
        dcsCritic: state.gatherCriticModel,
        criticModel: modelId,
